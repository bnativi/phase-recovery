...
def get_phase_noise(x,deviation): #this is the culmination of all previous work and is the desired alrogithm for iteratively determining phase with relative phase measurements
...
#     print(length)
    ...
    if (length==1): #if the vector has one entry, this is the base case and we either return the vector or in reality take an intensity measurement of that entry to deterimine the vector up to a global phase factor
        ...
        lst = []
        lst.append(intensity_measurement(x[0],deviation)) #this is the same as squaring the one entry
#         lst.append(x[0] + complex_random_noise(deviation)) #this is the same as squaring the one entry
#         print(lst)
        return lst
...
#     a = length/2   #Note that these are the only two choice of a that will always have the property that a*a=1
    ...
    recurse_vec = get_phase_noise(halfVec,deviation)      #recursive call on the half of x corresponding to Omega_a
    rel_phase_vec = relative_phase_noise(x,a,m,deviation) #get the relative phase measurements
    ...
    if (count != length/2):     #we compute phase by using the relative phase measurements from the second half of rel_phase_vec
        print("ERROR get_phase_noise: length of vector is odd")                 
